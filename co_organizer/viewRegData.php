...
$coOrganizerUsername = $_SESSION['username'];
$coOrganizer = $coOrganizerCollection->findOne(['username' => $coOrganizerUsername]);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action'])) {
    ...
    if ($action === 'Approve') {
        ...
        try {
            ...
            // Recipients
            $mail->setFrom($coOrganizerEmail, 'CampusConnect Admin');
            $mail->addAddress($participantEmail);

            // Email content
            $mail->isHTML(true);
            $mail->Subject = 'CampusConnect - Registration Approved';
            $mail->Body = "Dear {$participant['name']},<br><br>Your registration for the event has been approved.<br>Best Regards,<br>CampusConnect Team<br>Co Organizer Approved by: {$coOrganizerUsername}";
        ...
        try {
            ...
            $mail->Body = "Dear {$participant['name']},<br><br>Your registration for the event has been rejected for the following reason:<br><br><strong style='color: red;'>{$reason}</strong><br><br>Co Organizer Rejected by: {$coOrganizerUsername}";
